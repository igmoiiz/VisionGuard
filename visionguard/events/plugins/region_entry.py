...
class RegionEntryPlugin(BaseEventPlugin):
    def __init__(self, plugin_config=None) -> None:
        super().__init__("region_entry", plugin_config)
    ...
    def evaluate(self, tracks: List[Track], frame_data: FrameData, regions: List[Region]) -> List[Event]:
        ...
        for track in tracks:
            for region in poly_regions:
                ...
                if is_inside and key not in self.inside_state:
                ...
                elif not is_inside and key in self.inside_state:
